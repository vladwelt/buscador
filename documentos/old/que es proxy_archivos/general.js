...
function quickquotesel() {
	if (document.getSelection && document.getSelection().length > 0) {
 		qqsel = document.getSelection();
 	} else if (document.selection && document.selection.type == "Text" && document.selection.createRange) {
  		qqsel = document.selection.createRange().text;
  	}
}
...
function quickquote(pid) {
	...
	document.input.message.value += "[quote=" + username + "]" + quote + "[/quote]\n";
